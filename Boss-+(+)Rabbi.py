...
def wait():
	for i in range(100):
		print(f"\n\n{g}[ + ] অপেক্ষাকর মাদারচোদ Wait 120 second For Next Submit : {i+1}/120", end='\r')
		time.sleep(1)
		os.system('clear')
...
logo = (f"""
 ...
 █▀█ ▄▀█ █▄▄ █▄▄ █
 █▀▄ █▀█ █▄█ █▄█ █
 {c}Owner  : {ran}Owner Tajul Islam Rabbi
 {c}Chanel : {ran}T . I . R Cyber Team
 {c}Type   : {ran}খানকির পোলা টিকটক ভিও পাবি  TikTok View 🎁 
{ran}-----------------------------------{w}""")
def linex():
    print(f'{ran}-----------------------------------{w}')
...
def main():
	print(logo)
	print(f"{y} [√]ENTER YOUR TIKTOK USERNAME HERE 👇দেখ খানকির পোলা 👇{w}")
	linex()
	print(f"{y} [√]EXAMPLE >> মাদারচোদ দেখ👉 @rabbiislam1414👈{w}")
	linex()
